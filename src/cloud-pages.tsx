  ...
  return <><Header title="Planificacion cloud" subtitle="Diseña propuestas con una estimacion mensual inmediata." showPlanningSelector={false}/><main>
    <div className="planning-grid">
      <form className="panel form-panel" onSubmit={submit}>
        ...
        <div className={`region-capacity ${exceedsRegionCapacity ? 'warning' : ''}`}><Users size={17}/><div><div className="region-capacity-title"><b>{proposalRegion.code}: capacidad planificada de {proposalRegion.estimatedUsers.toLocaleString('en-US')} usuarios</b><StatusBadge status={proposalRegion.status} label={proposalRegion.status === 'healthy' ? 'Operativa' : 'En revision'}/></div><span>{exceedsRegionCapacity ? `La estimacion supera la capacidad del modelo por ${(estimatedUsers - proposalRegion.estimatedUsers).toLocaleString('en-US')} usuarios. Considera Multi-region.` : `La carga estimada utiliza ${Math.round(estimatedUsers / proposalRegion.estimatedUsers * 100)}% de la capacidad planificada.`}</span><small>{proposalRegion.recommendation}</small></div></div>
